/**
 * CM0102 in-game attribute display math (aligned with `DataService` / CM Scout Intrinsic).
 * Shared so the renderer attribute editor can preview values without duplicating formulas.
 */
...
function isGk(p: { goalkeeper: number }): boolean {
  return p.goalkeeper > 14
}

export function inGameCa18(idx: number, ca: number, intrinsic: number, p: { goalkeeper: number }): number {
  const gk = isGk(p)
...
export function inGameCa18Uncapped(idx: number, ca: number, intrinsic: number, p: { goalkeeper: number }): number {
  const gk = isGk(p)
...
/** Minimal player slice for CA18 math (all CA18 keys + CA + GK flag). */
export type PlayerAttrMathInput = { current_ability: number; goalkeeper: number } & Record<string, number>

export function buildCa18Display(p: PlayerAttrMathInput): Record<Ca18Key, AttrDisplayBlock> {
  const ca = p.current_ability
  const out = {} as Record<Ca18Key, AttrDisplayBlock>
  CA18_KEYS.forEach((key, idx) => {
    const raw = Number(p[key])
    const intrinsic = Number.isFinite(raw) ? raw : 0
    out[key] = {
      raw: intrinsic,
      inGame: inGameCa18(idx, ca, intrinsic, p),
      inGameUncapped: inGameCa18Uncapped(idx, ca, intrinsic, p),
      inMatch: inMatchValue(ca, intrinsic),
    }
  })
  return out
}
...
export function otherAttrDisplay(raw: number): AttrDisplayBlock {
  ...
  const ig = clamp20(r)
  return { raw: r, inGame: ig, inGameUncapped: r, inMatch: ig }
}
